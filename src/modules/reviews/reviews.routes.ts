...
reviewsRouter.post(
  ...
  asyncHandler(async (request, response) => {
    ...
    if (!appointment?._id) {
      throw conflict("Only your completed appointment can be reviewed", "REVIEW_NOT_ELIGIBLE");
    }
    const now = new Date();
    const review: Review = {
    ...
    try {
      ...
      if (error instanceof MongoServerError && error.code === 11000) {
        throw conflict("This appointment has already been reviewed", "DUPLICATE_REVIEW");
      }
      throw error;
    }
  }),
