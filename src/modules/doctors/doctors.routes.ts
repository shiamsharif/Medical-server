...
const updateSchema = z.object({
  ...
  availableDays: z.array(z.enum(["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"])).optional(),
}).refine((value) => Object.keys(value).length > 0, "At least one field is required");

const sortMap: Record<z.infer<typeof searchSchema>["sort"], Sort> = {
  fee_asc: { consultationFee: 1 }, fee_desc: { consultationFee: -1 },
  experience_desc: { experience: -1 }, rating_desc: { averageRating: -1 },
};

export const doctorsRouter = Router();

doctorsRouter.get("/", asyncHandler(async (request, response) => {
  const query = searchSchema.parse(request.query);
  const filter: Filter<Doctor> = { verificationStatus: VERIFICATION_STATUS.VERIFIED };
  if (query.search) filter.$text = { $search: query.search };
  if (query.specialization) filter.specialization = { $regex: query.specialization.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), $options: "i" };
  if (query.hospital) filter.hospitalName = { $regex: query.hospital.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), $options: "i" };
  const collection = getDatabase().collection<Doctor>("doctors");
  const [data, total] = await Promise.all([
    collection.find(filter).sort(sortMap[query.sort]).skip((query.page - 1) * query.limit).limit(query.limit).toArray(),
    collection.countDocuments(filter),
  ]);
  response.json({ success: true, data, meta: paginationMeta(query.page, query.limit, total) });
}));

doctorsRouter.get("/me/profile", authenticate, requireActiveUser, requireRole(USER_ROLE.DOCTOR), asyncHandler(async (request, response) => {
...
doctorsRouter.patch("/me/profile", authenticate, requireActiveUser, requireRole(USER_ROLE.DOCTOR), validateBody(updateSchema), asyncHandler(async (request, response) => {
  const result = await getDatabase().collection<Doctor>("doctors").findOneAndUpdate(
    { userId: request.principal!.appUserId }, { $set: { ...request.body, updatedAt: new Date() } }, { returnDocument: "after" },
  );
  if (!result) throw notFound("Doctor profile");
  success(response, result, "Doctor profile updated");
}));

doctorsRouter.get("/:id", asyncHandler(async (request, response) => {
  ...
  const [reviews, schedules] = await Promise.all([
    getDatabase().collection<Review>("reviews").find({ doctorId }).sort({ createdAt: -1 }).limit(20).toArray(),
    getDatabase().collection<Schedule>("schedules").find({ doctorId, active: true }).project({ doctorId: 0, createdAt: 0, updatedAt: 0 }).sort({ day: 1, startTime: 1 }).toArray(),
  ]);
