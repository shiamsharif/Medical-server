...
function reminderEmailHtml(appointment: ReminderCandidate): string {
  return [
    `<p>Hello ${appointment.patient.name},</p>`,
    `<p>Your appointment with ${appointment.doctor.doctorName} is tomorrow, `,
    `${appointment.appointmentDate} at ${appointment.appointmentTime}, `,
    `at ${appointment.doctor.hospitalName}.</p>`,
...
remindersRouter.post(
  ...
  asyncHandler(async (request, response) => {
    if (!validCronSecret(request.header("x-cron-secret"))) {
      throw forbidden("Invalid scheduler credential");
    }
    const tomorrow = new Date(Date.now() + 86_400_000).toISOString().slice(0, 10);
    const candidates = await getDatabase()
      .collection<Appointment>("appointments")
      .aggregate<ReminderCandidate>([
        {
          $match: {
            appointmentDate: tomorrow,
            appointmentStatus: APPOINTMENT_STATUS.ACCEPTED,
    ...
    for (const appointment of candidates) {
      const claimed = await getDatabase()
        .collection<Appointment>("appointments")
        .updateOne(
          { _id: appointment._id!, reminderSentAt: { $exists: false } },
      ...
      try {
        await resend.emails.send({
          from: requireIntegration("EMAIL_FROM"),
          to: appointment.patient.email,
          subject: "Your MediCare Connect appointment is tomorrow",
          html: reminderEmailHtml(appointment),
        });
        sent += 1;
      } catch (error) {
        ...
        await getDatabase()
          .collection<Appointment>("appointments")
          .updateOne({ _id: appointment._id! }, { $unset: { reminderSentAt: "" } });
