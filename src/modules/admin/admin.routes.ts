...
for (const [path, status] of [
  ...
  adminRouter.patch(
    ...
    asyncHandler(async (request, response) => {
      ...
      if (id.equals(request.principal!.appUserId) && status === USER_STATUS.SUSPENDED) {
        throw conflict("Administrators cannot suspend themselves", "SELF_SUSPENSION");
      }
      ...
      if (status === USER_STATUS.SUSPENDED) {
        await auth.api.banUser({
          body: { userId: target.authUserId, banReason: "Suspended by MediCare administrator" },
          headers: fromNodeHeaders(request.headers),
        });
      } else {
        await auth.api.unbanUser({
          body: { userId: target.authUserId },
          headers: fromNodeHeaders(request.headers),
        });
      }
      ...
      logger.info(
        { actorId: request.principal!.appUserId, targetUserId: id, status },
...
adminRouter.get(
  ...
  asyncHandler(async (request, response) => {
    const query = paginationSchema
      .extend({ status: z.nativeEnum(VERIFICATION_STATUS).optional() })
      .parse(request.query);
...
for (const [path, status] of [
  ...
  ["revoke", VERIFICATION_STATUS.PENDING],
] as const) {
  adminRouter.patch(
    `/doctors/:id/${path}`,
    asyncHandler(async (request, response) => {
      ...
      logger.info(
        { actorId: request.principal!.appUserId, doctorId: id, verificationStatus: status },
        "Admin changed doctor verification",
      );
...
adminRouter.get(
  ...
  asyncHandler(async (request, response) => {
    ...
    if (query.search) {
      filter.$or = [
        { symptoms: { $regex: query.search, $options: "i" } },
        { appointmentDate: query.search },
      ];
      if (ObjectId.isValid(query.search)) {
        filter.$or.push({ _id: new ObjectId(query.search) });
      }
    }
    if (query.status) {
...
adminRouter.get(
  ...
  asyncHandler(async (request, response) => {
    const query = paginationSchema
      .extend({ status: z.nativeEnum(PAYMENT_STATUS).optional() })
      .parse(request.query);
...
adminRouter.delete(
  ...
  asyncHandler(async (request, response) => {
    ...
    if (user.role === USER_ROLE.ADMIN) {
      throw badRequest(
        "Administrator deletion requires a dedicated operational process",
        "UNSAFE_DELETE",
      );
    }
    const doctor =
    ...
    if (doctor?._id) {
      ownershipFilters.push({ doctorId: doctor._id });
    }
    const hasRecords = await getDatabase()
      .collection<Appointment>("appointments")
      .countDocuments({ $or: ownershipFilters });
    if (hasRecords) {
      throw conflict(
        "User with healthcare records cannot be deleted; suspend the account instead",
        "USER_HAS_RECORDS",
      );
    }
    await auth.api.removeUser({
      body: { userId: user.authUserId },
      headers: fromNodeHeaders(request.headers),
    });
    if (doctor?._id) {
      await getDatabase().collection<Doctor>("doctors").deleteOne({ _id: doctor._id });
    }
